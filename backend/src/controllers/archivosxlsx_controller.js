...
const importarExcel = (req,res) =>{
    try {
        ...
        if(!ext_permitidas.includes(ext)){
            ...
            resp_json.mensaje += "El archivo " + f_subir.name + " es un archivo no permitido"
        ...
        f_subir.mv(ruta_archivo, (err) =>{
            if(err){
                return res.status(200).json({mensaje: "Ocurrió un error al subir el archivo: " + err})
            }
            req.f_subir = f_subir
            return res.status(200).json({mensaje: "Archivo Cargado", archivo: f_subir.name})
        })
    } catch (error) {
...
const subirBDD = async (req, res) => {
    if (f_subir) {
        await Comisiones.collection.drop()
        ...
        let comisiones_formateadas = jsa.map(comision => {
            let obj = {};
...
const exportarExcel = async (req,res) => {
    try {
        const comisionesBDD = await Comisiones.find().sort('etapa manzana lote').select('-_id -__v').lean();
        let comisiones_formateadas = comisionesBDD.map(comision => {
            ...
            if(comision["abonos_anteriores"].length>0){
                ...
                obj[nombres_excel.fecha_abono] = fechas_concat + comision["fecha_ultimo_abono"]
            }else{
        ...
        comisiones_formateadas = comisiones_formateadas.filter(item => typeof item["ETAPA"] !== "undefined")
        ...
        res.setHeader('Content-Disposition', `attachment; filename=Comisiones_${Date.now().toString()}.xlsx`);
