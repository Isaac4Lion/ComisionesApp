...
export default function InfoLot({ 
  ...
  const [valorTotalRecibido, setValorTotalRecibido] = useState(valorTotalRecibidoAnterior);
  ...
  const handleEditButton = async (e) => {
    ...
    if (edit) {
      if (valorTotalRecibidoAnterior != valorTotalRecibido) {
        try {
          const response = await fetch(`http://localhost:3000/api/lotes/${detalleLote.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ valor_total_recibido: Number(valorTotalRecibido) }),
          });
                    ...
                    <div className="mt-2">
                      <form className="w-full">
                        <div className="grid grid-cols-2 md:grid-cols-5 sm:grid-cols-3 -mx-3 gap-x-5">
                          {Object.entries(detalleLote).filter(key => key[0] !== 'id' && key[0] !== 'desistimiento' && key[0] !== '_id' && key[0] !== '__v').map(([key, value]) => (
                            <div className="mb-5" key={key}>
                              ...
                              <input
                                ...
                                onChange={key === 'valor_total_recibido' ? handleValorTotalRecibido : undefined}
                                ...
                                disabled={!(edit && key === 'valor_total_recibido')}
                              />
                            </div>
                          ))}
                        </div>
              ...
              <div className="flex justify-between w-full">
                ...
                <div className="px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  ...
                  <button
                    ...
                    className={`mt-3 inline-flex w-full justify-center rounded-md ${edit ? "bg-blue-800" : "bg-blue-600"} px-3 py-2 text-sm font-semibold text-white shadow-sm ring-1 ring-inset ring-gray-300 ${edit ? "hover:bg-blue-600" : "hover:bg-blue-400"} sm:ml-3 sm:mt-0 sm:w-auto`}
